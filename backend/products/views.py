...
class BrandBannerListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        if not BrandBanner.objects.exists():
            banners = [
            ...
            for b in banners:
                BrandBanner.objects.create(**b)
        return BrandBanner.objects.all()
    ...
    def get_queryset(self):
        if not HeroBanner.objects.exists():
            ...
            HeroBanner.objects.create(
                ...
                subtitle="Get this bestseller now at an unbeatable price! Only COD and free shipping above ₹199.",
